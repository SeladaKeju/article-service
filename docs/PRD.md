...
- IDs are lowercase UUID v4 strings in the API and stored as `text`, following the assessment diagram. Generate article IDs and timestamps server-side; return UTC RFC 3339 timestamps with six fractional digits matching stored precision.
- Responses: `201` with an article object; `200` with `{items: [...], meta: {next_cursor: string|null}}`, without total count. Empty results use `items: []`. Errors use `{code, message}`: `400` for `invalid_request`, `author_not_found`, `invalid_limit`, or `invalid_cursor`; `500 internal_error` without SQL details for internal/database failures.
